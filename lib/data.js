...
//import { getGenre, insertGenre, updateGenre } from "./db/users";
...
/*
function saveAnimeGenre(anime) {
  anime.genres.forEach(async (genre) => {
    const existingGenre = await getGenre(genre.id);
    if (existingGenre) {
      if (
        existingGenre.anime_ids &&
        !existingGenre.anime_ids.includes(anime._id)
      ) {
        updateGenre(genre.id, anime._id);
      }
    } else {
      const newGenre = {
        _id: genre.id,
