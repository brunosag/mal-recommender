...
export function formatYearIntervalChip(yearInterval) {
  return yearInterval.initial_year.toString() + ' – ' + yearInterval.final_year.toString();
}
...
function sortAnimeRecommendationsList(animeRecommendationsList, sortType, sortIncreasing) {
  if (sortType.has('points'))
    return animeRecommendationsList.sort((a, b) => (sortIncreasing ? a.points - b.points : b.points - a.points));
  if (sortType.has('title'))
    return animeRecommendationsList.sort((a, b) =>
      sortIncreasing ? a.title.jp.localeCompare(b.title.jp) : b.title.jp.localeCompare(a.title.jp)
    );
  if (sortType.has('mean'))
    return animeRecommendationsList.sort((a, b) => (sortIncreasing ? a.mean - b.mean : b.mean - a.mean));
  if (sortType.has('members'))
    return animeRecommendationsList.sort((a, b) => (sortIncreasing ? a.members - b.members : b.members - a.members));
...
function filterAnimeRecommendationsList(animeRecommendationsList, filters, filtersCollections) {
  // Filter between categories (intersection)
  ...
  let mediaTypesFilteredAnimeIdsSet = new Set();
  if (filteredMediaTypes.length === 0) {
    mediaTypesFilteredAnimeIdsSet = new Set([...allFilteredAnimeIdsSet]);
  }
  for (const mediaType of filteredMediaTypes) {
    const animeIdsMediaTypeSet = new Set([...mediaType.anime_ids]);
    mediaTypesFilteredAnimeIdsSet = union(mediaTypesFilteredAnimeIdsSet, animeIdsMediaTypeSet);
  }
  allFilteredAnimeIdsSet = intersection(allFilteredAnimeIdsSet, mediaTypesFilteredAnimeIdsSet);

  // Filter by years (union)
...
export function prepareAnimeRecommendationsList({ animeRecommendationsList, filters, filtersCollections, sortType }) {
  ...
  return sortAnimeRecommendationsList(filteredAnimeRecommendationsList, sortType.criteria, sortType.increasing);
}

export function getMaxCount(data, setMaxCount) {
  for (const entry of data) {
    const count = entry.anime_ids.length;
